...
DECLARE
    PROCEDURE insert_reference(
        ...
        p_editable    IN CHAR     := 'N'
    ) IS
    BEGIN
        INSERT INTO tbom_reference_data (ref_data_type, ref_data_name, editable, description, effect_from_dat, effect_to_dat)
        SELECT p_type, p_name, p_editable, p_description, p_effect_from, p_effect_to
        FROM DUAL
...
BEGIN
    ...
    insert_reference('METADATA_KEY', 'REQUEST_CORRELATION_ID', 'Metadata key for request correlation id', p_editable => 'Y');
    insert_reference('METADATA_KEY', 'SOURCE_ENVIRONMENT', 'Metadata key for source environment', p_editable => 'Y');
    insert_reference('METADATA_KEY', 'LANGUAGE_CODE', 'Metadata key for language code', p_editable => 'Y');
    insert_reference('METADATA_KEY', 'FOOTER_ID', 'Metadata key for policy number', p_editable => 'Y');
    insert_reference('METADATA_KEY', 'ATRADIUS_ORG_ID', 'Metadata key for Atradius organization identifier', p_editable => 'Y');
    insert_reference('METADATA_KEY', 'CUSTOMER_ID', 'Metadata key for customer identifier', p_editable => 'Y');
    insert_reference('METADATA_KEY', 'POLICY_ID', 'Metadata key for policy number', p_editable => 'Y');
    insert_reference('METADATA_KEY', 'INVOICE_ID', 'Metadata key for policy number', p_editable => 'Y');
    ...
    insert_reference('FOOTER_ID', '0', 'Default footer for documents', p_editable => 'Y');
    insert_reference('FOOTER_ID', '1', 'Footer id 1 for documents', p_editable => 'Y');
    insert_reference('FOOTER_ID', '2', 'Footer id 2 for documents', p_editable => 'Y');
    insert_reference('FOOTER_ID', '3', 'Footer id 3 for documents', p_editable => 'Y');
    insert_reference('FOOTER_ID', '4', 'Footer id 4 for documents', p_editable => 'Y');

    -- Document Configuration Codes (editable)
    insert_reference('DOC_CONFIG_CODE', 'SIGNEE_1', 'Footer reference code for signee 1', p_editable => 'Y');
    insert_reference('DOC_CONFIG_CODE', 'SIGNEE_2', 'Footer reference code for signee 2', p_editable => 'Y');
    insert_reference('DOC_CONFIG_CODE', 'TOPIC_NAME', 'Footer reference code for topic name', p_editable => 'Y');
    insert_reference('DOC_CONFIG_CODE', 'BATCH_CONFIG_ID', 'Thunderhead batch config id', p_editable => 'Y');
