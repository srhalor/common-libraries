...
-- Purpose: Utility function to resolve the active tbom_reference_data.id for a given (type, name)
...
BEGIN
    ...
    FROM (SELECT id
          ...
          WHERE ref_data_type = p_ref_data_type
            AND ref_data_name = p_ref_data_name
    ...
    WHEN NO_DATA_FOUND THEN
        IF p_required = 1 THEN
            RAISE_APPLICATION_ERROR(-20002,
                                    'Reference not found for ' || p_ref_data_type || ' / ' || p_ref_data_name);
        END IF;
        RETURN NULL;
    WHEN OTHERS THEN
        IF p_required = 1 THEN
